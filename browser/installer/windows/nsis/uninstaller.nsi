...
Function un.UninstallServiceIfNotUsed
  ...
  StrCpy $0 0
  ${Do}
    EnumRegKey $1 HKLM "Software\Mypal68\MaintenanceService" $0
    ${If} "$1" == ""
      ${ExitDo}
    ${EndIf}
    IntOp $0 $0 + 1
...
Section "Uninstall"
  ...
  ${un.InitHashAppModelId} "$INSTDIR" "Software\Mypal68\${AppName}\TaskBarIDs"

  SetShellVarContext current  ; Set SHCTX to HKCU
  ${un.RegCleanMain} "Software\Mypal68"
  ...
  DeleteRegValue HKCU "Software\Mypal68\${AppName}\TaskBarIDs" "$INSTDIR"
  DeleteRegValue HKLM "Software\Mypal68\${AppName}\TaskBarIDs" "$INSTDIR"
  ...
  SetShellVarContext all  ; Set SHCTX to HKLM
  ${un.GetSecondInstallPath} "Software\Mypal68" $R9
  ${If} $R9 == "false"
    SetShellVarContext current  ; Set SHCTX to HKCU
    ${un.GetSecondInstallPath} "Software\Mypal68" $R9
  ${EndIf}
...
!endif
  ...
  WriteRegStr HKCU "Software\Mypal68" "Uninstalled-${UpdateChannel}" "True"
