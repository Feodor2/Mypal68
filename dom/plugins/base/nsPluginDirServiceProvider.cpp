...
static BOOL FileExists(LPCWSTR szFile) {
  return GetFileAttributesW(szFile) != 0xFFFFFFFF;
}

// Get file version information from a file
static BOOL GetFileVersion(LPCWSTR szFile, verBlock* vbVersion) {
  UINT uLen;
  UINT dwLen;
  BOOL bRv;
  DWORD dwHandle;
  ...
  if (FileExists(szFile)) {
    ...
    if (lpData &&
        GetFileVersionInfoW(lpFilepath, dwHandle, dwLen, lpData) != 0) {
...
static void TranslateVersionStr(const WCHAR* szVersion, verBlock* vbVersion) {
...
nsPluginDirServiceProvider::GetFile(const char* charProp, bool* persistant,
  ...
  *persistant = false;
  ...
  if (nsCRT::strcmp(charProp, NS_WIN_QUICKTIME_SCAN_KEY) == 0) {
    nsAutoCString strVer;
    Preferences::GetCString(charProp, strVer);
    if (strVer.IsEmpty()) {
    ...
    rv =
        regKey->Open(nsIWindowsRegKey::ROOT_KEY_LOCAL_MACHINE,
                     NS_LITERAL_STRING("software\\Microsoft\\Windows\\CurrentVe"
                                       "rsion\\App Paths\\QuickTimePlayer.exe"),
    ...
    if (CompareVersion(qtVer, minVer) < 0) return rv;
    ...
    rv = regKey->Open(nsIWindowsRegKey::ROOT_KEY_LOCAL_MACHINE,
                      NS_LITERAL_STRING("software\\Microsoft\\Windows\\CurrentV"
                                        "ersion\\App Paths\\wmplayer.exe"),
                      nsIWindowsRegKey::ACCESS_READ);
    if (NS_SUCCEEDED(rv)) {
      nsAutoString path;
      rv = regKey->ReadStringValue(NS_LITERAL_STRING(""), path);
    ...
    if (CompareVersion(wmpVer, minVer) < 0) return rv;
    ...
    if (!newestPath.IsEmpty()) {
      ...
      rv = NS_NewLocalFile(newestPath, true, getter_AddRefs(localFile));
    }
  }
