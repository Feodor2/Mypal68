...
CompositorD3D9::CompositorD3D9(CompositorBridgeParent *aParent,
                               widget::CompositorWidget *aWidget)
    : Compositor(aWidget, aParent), mDeviceResetCount(0),
...
bool CompositorD3D9::Initialize(nsCString *const out_failureReason) {
...
TextureFactoryIdentifier CompositorD3D9::GetTextureFactoryIdentifier() {
...
bool CompositorD3D9::CanUseCanvasLayerForSize(const IntSize &aSize) {
...
int32_t CompositorD3D9::GetMaxTextureSize() const {
...
CompositorD3D9::CreateDataTextureSource(TextureFlags aFlags) {
  return MakeAndAddRef<DataTextureSourceD3D9>(this->GetD3D9Device(),
                                              SurfaceFormat::UNKNOWN, aFlags);
}
...
CompositorD3D9::CreateRenderTarget(const gfx::IntRect &aRect,
  ...
  HRESULT hr = GetD3D9Device()->CreateTexture(
      aRect.width, aRect.height, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8,
      D3DPOOL_DEFAULT, getter_AddRefs(texture), nullptr);
  if (FAILED(hr)) {
    ReportFailure(
        NS_LITERAL_CSTRING(
            "CompositorD3D9::CreateRenderTarget: Failed to create texture"),
        hr);
    return nullptr;
  }
...
CompositorD3D9::CreateTexture(const gfx::IntRect &aRect,
  ...
  if (FAILED(hr)) {
    ReportFailure(
        NS_LITERAL_CSTRING("CompositorD3D9::CreateRenderTargetFromSource: "
  ...
  if (aSource) {
    RefPtr<IDirect3DSurface9> sourceSurface =
        static_cast<const CompositingRenderTargetD3D9 *>(aSource)
            ->GetD3D9Surface();
    ...
    if (sourceSurface && destSurface) {
      ...
      hr = GetD3D9Device()->StretchRect(sourceSurface, &sourceRect, destSurface,
...
CompositorD3D9::CreateRenderTargetFromSource(
    ...
    const gfx::IntPoint &aSourcePoint) {
  RefPtr<IDirect3DTexture9> texture =
      CreateTexture(aRect, aSource, aSourcePoint);

  if (!texture) {
    return nullptr;
  }

  return MakeAndAddRef<CompositingRenderTargetD3D9>(texture, INIT_MODE_NONE,
                                                    aRect);
}

void CompositorD3D9::SetRenderTarget(CompositingRenderTarget *aRenderTarget) {
  ...
  mCurrentRT->BindRenderTarget(GetD3D9Device());
...
void CompositorD3D9::ClearRect(const gfx::Rect &aRect) {
  D3DRECT rect;
  rect.x1 = aRect.X();
  rect.y1 = aRect.Y();
  rect.x2 = aRect.XMost();
  ...
  GetD3D9Device()->Clear(1, &rect, D3DCLEAR_TARGET, 0x00000000, 0, 0);
}

void CompositorD3D9::DrawQuad(const gfx::Rect &aRect,
  ...
  d3d9Device->SetVertexShaderConstantF(CBvRenderTargetOffset,
                                       renderTargetOffset, 1);
  d3d9Device->SetVertexShaderConstantF(
      CBvLayerQuad,
      ShaderConstantRect(aRect.x, aRect.y, aRect.width, aRect.height), 1);
  ...
  if (aEffectChain.mSecondaryEffects[EffectTypes::BLEND_MODE]) {
    ...
    //  (3) Capture the backdrop again into another surface - these are our
    //  source pixels. (4) Perform a final blend step using software. (5) Blit
    //  the blended result back to the render target.
    if (BlendOpIsMixBlendMode(blendMode)) {
      backdropRect = ComputeBackdropCopyRect(aRect, aClipRect, aTransform,
                                             &backdropTransform, &backdropDest);

      // If this fails, don't set a blend op.
      backdropTexture =
          CreateTexture(backdropRect, mCurrentRT, backdropRect.TopLeft());
      if (!backdropTexture) {
  ...
  case EffectTypes::SOLID_COLOR: {
    ...
    maskTexture = mDeviceManager->SetShaderMode(
        DeviceManagerD3D9::SOLIDCOLORLAYER, maskType);
  } break;
  case EffectTypes::RENDER_TARGET:
  case EffectTypes::RGB: {
    ...
    d3d9Device->SetVertexShaderConstantF(
        CBvTextureCoords,
        ShaderConstantRect(textureCoords.x, textureCoords.y,
    ...
    maskTexture = mDeviceManager->SetShaderMode(
        ShaderModeForEffectType(aEffectChain.mPrimaryEffect->mType,
  ...
  case EffectTypes::YCBCR: {
    ...
    GetD3D9Device()->SetTexture(Y, sourceY->GetD3D9Texture());
    GetD3D9Device()->SetTexture(Cb, sourceCb->GetD3D9Texture());
    GetD3D9Device()->SetTexture(Cr, sourceCr->GetD3D9Texture());
    maskTexture =
  ...
  case EffectTypes::COMPONENT_ALPHA: {
    ...
    TextureSourceD3D9 *sourceOnWhite =
        effectComponentAlpha->mOnWhite->AsSourceD3D9();
    TextureSourceD3D9 *sourceOnBlack =
        effectComponentAlpha->mOnBlack->AsSourceD3D9();
    ...
    maskTexture = mDeviceManager->SetShaderMode(
        DeviceManagerD3D9::COMPONENTLAYERPASS1, maskType);
  ...
  // source textures.
  if (BlendOpIsMixBlendMode(blendMode)) {
    FinishMixBlend(backdropRect, backdropDest, backdropTransform,
                   backdropTexture, blendMode);
  }
}

void CompositorD3D9::SetMask(const EffectChain &aEffectChain,
                             uint32_t aMaskTexture) {
  ...
  GetD3D9Device()->SetTexture(aMaskTexture, source->GetD3D9Texture());
  ...
  GetD3D9Device()->SetVertexShaderConstantF(
      DeviceManagerD3D9::sMaskQuadRegister,
      ShaderConstantRect(bounds.x, bounds.y, bounds.width, bounds.height), 1);
}

/**
 * In the next few methods we call |mParent->InvalidateRemoteLayers()| - that
 * has a few uses - if our device or swap chain is not ready, it causes us to
 * try to render again, that means we keep trying to get a good device and swap
 ...
 * Currently we probably do this complete invalidation too much. But it is
 * better to do that than to miss an invalidation which would result in a black
 * layer (or multiple layers) until the user moves the mouse. The unnecessary
 * invalidtion only happens when the device is reset, so that should be pretty
 * rare and when other things are happening so the user does not expect super
 * performance.
 */

bool CompositorD3D9::EnsureSwapChain() {
  MOZ_ASSERT(mDeviceManager,
             "Don't call EnsureSwapChain without a device manager");

  if (!mSwapChain) {
    mSwapChain =
        mDeviceManager->CreateSwapChain(mWidget->AsWindows()->GetHwnd());
...
void CompositorD3D9::CheckResetCount() {
...
bool CompositorD3D9::Ready() {
  ...
  NS_ASSERTION(!mCurrentRT && !mDefaultRT,
               "Shouldn't have any render targets around, they must be "
               "released before our device");
  mSwapChain = nullptr;
...
void CompositorD3D9::PrepareViewport(const gfx::IntSize &aSize) {
  ...
  HRESULT hr = GetD3D9Device()->SetVertexShaderConstantF(CBmProjection,
                                                         &viewMatrix._11, 4);
...
bool CompositorD3D9::SupportsEffect(EffectTypes aEffect) {
...
void CompositorD3D9::EnsureSize() { mSize = mWidget->GetClientSize(); }

void CompositorD3D9::SetSamplerForSamplingFilter(
    SamplingFilter aSamplingFilter) {
  switch (aSamplingFilter) {
  case SamplingFilter::LINEAR:
    GetD3D9Device()->SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_LINEAR);
    GetD3D9Device()->SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_LINEAR);
    return;
...
void CompositorD3D9::PaintToTarget() {
  if (!mDeviceManager) {
    return;
  }
  ...
  GetD3D9Device()->GetRenderTarget(0, getter_AddRefs(backBuff));

  D3DSURFACE_DESC desc;
  backBuff->GetDesc(&desc);

  GetD3D9Device()->CreateOffscreenPlainSurface(
      desc.Width, desc.Height, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM,
      getter_AddRefs(destSurf), nullptr);

  GetD3D9Device()->GetRenderTargetData(backBuff, destSurf);

  D3DLOCKED_RECT rect;
  HRESULT hr = destSurf->LockRect(&rect, nullptr, D3DLOCK_READONLY);
  if (FAILED(hr) || !rect.pBits) {
    gfxCriticalError() << "Failed to lock rect in paint to target D3D9 "
                       << hexa(hr);
    return;
  }
  RefPtr<DataSourceSurface> sourceSurface =
      Factory::CreateWrappingDataSourceSurface(
          (uint8_t *)rect.pBits, rect.Pitch, IntSize(desc.Width, desc.Height),
          SurfaceFormat::B8G8R8A8);
  mTarget->CopySurface(sourceSurface, IntRect(0, 0, desc.Width, desc.Height),
                       IntPoint(-mTargetBounds.x, -mTargetBounds.y));
  mTarget->Flush();
  destSurf->UnlockRect();
}

void CompositorD3D9::ReportFailure(const nsACString &aMsg, HRESULT aCode) {
...
CreateDataSurfaceForTexture(IDirect3DDevice9 *aDevice,
  ...
  HRESULT hr = aDevice->CreateOffscreenPlainSurface(
      aDesc.Width, aDesc.Height, aDesc.Format, D3DPOOL_SYSTEMMEM,
...
public:
  ...
  bool Okay() const { return !!mSurface; }
  int Pitch() const {
    MOZ_ASSERT(Okay());
    return mRect.Pitch;
  }
...
void CompositorD3D9::FinishMixBlend(const gfx::IntRect &aBackdropRect,
  ...
  // New scope for locks and temporary surfaces.
  {
    ...
    RefPtr<DrawTarget> dest = Factory::CreateDrawTargetForData(
        BackendType::CAIRO, backdropLock.Bits(),
        gfx::IntSize(backdropDesc.Width, backdropDesc.Height),
        backdropLock.Pitch(), SurfaceFormat::B8G8R8A8);

    // The backdrop rect is rounded out - account for any difference between
    // it and the actual destination.
    gfx::Rect destRect(aBackdropDest.x - aBackdropRect.x,
                       aBackdropDest.y - aBackdropRect.y, aBackdropDest.width,
                       aBackdropDest.height);

    dest->DrawSurface(source, destRect, destRect, gfx::DrawSurfaceOptions(),
  ...
  RECT destRect = {aBackdropRect.x, aBackdropRect.y, aBackdropRect.XMost(),
                   aBackdropRect.YMost()};
  hr = d3d9Device->StretchRect(backdropSurface, nullptr,
                               mCurrentRT->GetD3D9Surface(), &destRect,
                               D3DTEXF_NONE);
  if (FAILED(hr)) {
