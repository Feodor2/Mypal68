...
public:
  CompositorD3D9(CompositorBridgeParent *aParent,
  ...
  IDirect3DDevice9 *GetD3D9Device() const override {
   ...
   * resetting/recreating devices and swap chains. That is handled by this
   * method. If we don't have a device and swap chain ready for rendering, we
   * will return false and if necessary destroy the device and/or swap chain. We
   * will also schedule another composite so we get another go at rendering,
   * thus we shouldn't miss a composite due to re-creating a device.
   */
