...
bool SwapChainD3D9::Init(HWND hWnd) {
  RECT r;
  ...
  HRESULT hr = mDeviceManager->device()->CreateAdditionalSwapChain(
      &pp, getter_AddRefs(mSwapChain));
...
already_AddRefed<IDirect3DSurface9> SwapChainD3D9::GetBackBuffer() {
  RefPtr<IDirect3DSurface9> backBuffer;
  mSwapChain->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO,
                            getter_AddRefs(backBuffer));
  return backBuffer.forget();
}

DeviceManagerState SwapChainD3D9::PrepareForRendering() {
  RECT r;
...
void SwapChainD3D9::Present(const gfx::IntRect &aRect) {
  RECT r;
  r.left = aRect.x;
  r.top = aRect.y;
  r.right = aRect.XMost();
...
void SwapChainD3D9::Present() {
  mSwapChain->Present(nullptr, nullptr, 0, 0, 0);
}

void SwapChainD3D9::Reset() { mSwapChain = nullptr; }
...
DeviceManagerD3D9::DeviceManagerD3D9()
    : mTextureHostList(nullptr), mDeviceResetCount(0), mMaxTextureSize(0),
      mTextureAddressingMode(D3DTADDRESS_CLAMP), mHasComponentAlpha(true),
      mHasDynamicTextures(false), mDeviceWasRemoved(false) {}

DeviceManagerD3D9::~DeviceManagerD3D9() { DestroyDevice(); }

/* static */ RefPtr<DeviceManagerD3D9> DeviceManagerD3D9::Get() {
  MutexAutoLock lock(*sDeviceManagerLock);

  bool canCreate = !gfxPlatform::UsesOffMainThreadCompositing() ||
...
/* static */ RefPtr<IDirect3DDevice9> DeviceManagerD3D9::GetDevice() {
  MutexAutoLock lock(*sDeviceManagerLock);
...
bool DeviceManagerD3D9::Initialize() {
  WNDCLASSW wc;
  HRESULT hr;
  ...
  decltype(Direct3DCreate9Ex) *d3d9CreateEx =
      (decltype(Direct3DCreate9Ex) *)GetProcAddress(d3d9, "Direct3DCreate9Ex");
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << "[D3D9] Failed to create the environment code: "
                       << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg
                       << "LayerQuadVS: " << gfx::hexa(hr);
    return false;
  }

  hr = mDevice->CreatePixelShader((DWORD *)RGBShaderPS, getter_AddRefs(mRGBPS));

  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg
                       << "RGBShaderPS: " << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg
                       << "ComponentPass1ShaderPS: " << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg
                       << "ComponentPass2ShaderPS: " << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg << "SolidColorShaderPS"
                       << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg << "RGBShaderPSMask "
                       << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg
                       << "ComponentPass1ShaderPSMask: " << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << failCreateShaderMsg
                       << "SolidColorShaderPSMask: " << gfx::hexa(hr);
    return false;
  }
  ...
  if (FAILED(hr)) {
    gfxCriticalError() << "[D3D9] Failed to set the stream source code: "
                       << gfx::hexa(hr);
    return false;
  }

  D3DVERTEXELEMENT9 elements[] = {{0, 0, D3DDECLTYPE_FLOAT2,
  ...
  if (console) {
    ...
    msg += NS_LITERAL_STRING("\nVersion: ");
    msg += NS_ConvertUTF8toUTF16(nsPrintfCString(
        "%d.%d.%d.%d", HIWORD(identifier.DriverVersion.HighPart),
...
void DeviceManagerD3D9::SetupRenderState() {
...
already_AddRefed<SwapChainD3D9> DeviceManagerD3D9::CreateSwapChain(HWND hWnd) {
...
uint32_t DeviceManagerD3D9::SetShaderMode(ShaderMode aMode,
                                          MaskType aMaskType) {
...
void DeviceManagerD3D9::DestroyDevice() {
  ++mDeviceResetCount;
...
DeviceManagerState DeviceManagerD3D9::VerifyReadyForRendering() {
...
bool DeviceManagerD3D9::VerifyCaps() {
  D3DCAPS9 caps;
...
bool DeviceManagerD3D9::CreateVertexBuffer() {
  HRESULT hr;

  hr = mDevice->CreateVertexBuffer(sizeof(vertex) * 4, D3DUSAGE_WRITEONLY, 0,
                                   D3DPOOL_DEFAULT, getter_AddRefs(mVB),
                                   nullptr);
...
DeviceManagerD3D9::CreateTexture(const IntSize &aSize, _D3DFORMAT aFormat,
                                 D3DPOOL aPool,
  ...
  if (FAILED(device()->CreateTexture(aSize.width, aSize.height, 1, 0, aFormat,
                                     aPool, getter_AddRefs(result), nullptr))) {
    return nullptr;
  }

  NS_ASSERTION(aPool != D3DPOOL_MANAGED,
               "Should not be using MANAGED texture pool. We will get an error "
               "when we have to recreate the device");
  if (aPool == D3DPOOL_DEFAULT) {
    MOZ_ASSERT(
        aTextureHost,
        "We need a texture host to track so we can release the texture.");
...
void DeviceManagerD3D9::RegisterTextureHost(TextureSourceD3D9 *aHost) {
  if (!aHost) {
    return;
  }

  // Don't add aHost to the list twice.
  if (aHost->mPreviousHost || mTextureHostList == aHost) {
...
void DeviceManagerD3D9::ReleaseTextureResources() {
...
void DeviceManagerD3D9::RemoveTextureListHead(TextureSourceD3D9 *aHost) {
  MOZ_ASSERT(!aHost->mCreatingDeviceManager ||
                 aHost->mCreatingDeviceManager == this,
