...
DEFINE_GUID(CLSID_NV3DVStreaming, 0xf7747266, 0x777d, 0x4f61, 0xa1, 0x75, 0xdd,
            0x5a, 0xdf, 0x1e, 0x37, 0xdf);

DEFINE_GUID(IID_INV3DVStreaming, 0xf98f9bb2, 0xb914, 0x4d44, 0x98, 0xfa, 0x6e,
            0x37, 0x85, 0x16, 0x98, 0x55);
 ...
 */
Nv3DVUtils::Nv3DVUtils() : m3DVStreaming(nullptr) {}

Nv3DVUtils::~Nv3DVUtils() { UnInitialize(); }
...
void Nv3DVUtils::Initialize() {
   ...
   */
  HRESULT hr = CoCreateInstance(CLSID_NV3DVStreaming, nullptr,
                                CLSCTX_INPROC_SERVER, IID_INV3DVStreaming,
                                (void **)(getter_AddRefs(m3DVStreaming)));
  ...
  /*
   * Initialize the object. Note that m3DVStreaming cannot be nullptr at this
   * point.
   */
...
/**
 * Sets the device info, along with any other initialization that is needed
 * after device creation Pass the D3D9 device pointer is an IUnknown input
 * argument.
 */
void Nv3DVUtils::SetDeviceInfo(IUnknown *devUnknown) {
  if (!devUnknown) {
  ...
  rv = m3DVStreaming->Nv3DVControl(NV_STEREO_MODE_RIGHT_LEFT, true,
 ...
 */
void Nv3DVUtils::SendNv3DVControl(Nv_Stereo_Mode eStereoMode,
                                  bool bEnableStereo, DWORD dw3DVAppHandle) {
  if (!m3DVStreaming)
    return;

  DebugOnly<bool> rv =
      m3DVStreaming->Nv3DVControl(eStereoMode, bEnableStereo, dw3DVAppHandle);
  WARN_IF_FALSE(rv, "Nv3DVStreaming Nv3DVControl failed!");
}
 ...
 */
void Nv3DVUtils::SendNv3DVMetaData(unsigned int dwWidth, unsigned int dwHeight,
                                   HANDLE hSrcLuma, HANDLE hDst) {
  if (!m3DVStreaming)
    return;

  DebugOnly<bool> rv = m3DVStreaming->Nv3DVMetaData(
      (DWORD)dwWidth, (DWORD)dwHeight, hSrcLuma, hDst);
