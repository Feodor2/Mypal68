...
already_AddRefed<TextureHost> CreateTextureHostD3D9(
  ...
  switch (aDesc.type()) {
    ...
    case SurfaceDescriptor::TSurfaceDescriptorDXGIYCbCr: {
      result = new DXGIYCbCrTextureHostD3D9(
...
static SurfaceFormat D3D9FormatToSurfaceFormat(_D3DFORMAT format) {
  switch (format) {
...
static _D3DFORMAT SurfaceFormatToD3D9Format(SurfaceFormat format) {
  switch (format) {
...
CompositingRenderTargetD3D9::CompositingRenderTargetD3D9(
    IDirect3DTexture9* aTexture, SurfaceInitMode aInit,
    const gfx::IntRect& aRect)
    : CompositingRenderTarget(aRect.TopLeft()), mInitMode(aInit) {
...
CompositingRenderTargetD3D9::CompositingRenderTargetD3D9(
    IDirect3DSurface9* aSurface, SurfaceInitMode aInit,
    const gfx::IntRect& aRect)
...
void CompositingRenderTargetD3D9::BindRenderTarget(IDirect3DDevice9* aDevice) {
...
IntSize CompositingRenderTargetD3D9::GetSize() const {
 ...
 */
already_AddRefed<IDirect3DTexture9> TextureSourceD3D9::InitTextures(
    DeviceManagerD3D9* aDeviceManager, const IntSize& aSize, _D3DFORMAT aFormat,
    RefPtr<IDirect3DSurface9>& aSurface, D3DLOCKED_RECT& aLockedRect) {
  if (!aDeviceManager) {
    return nullptr;
  }
  ...
  if (FAILED(hr) || !aLockedRect.pBits) {
    gfxCriticalError() << "Failed to lock rect initialize texture in D3D9 "
                       << hexa(hr);
    return nullptr;
  }
 ...
 * Helper method for DataToTexture.
 */
static void FinishTextures(DeviceManagerD3D9* aDeviceManager,
...
already_AddRefed<IDirect3DTexture9> TextureSourceD3D9::DataToTexture(
    DeviceManagerD3D9* aDeviceManager, unsigned char* aData, int aStride,
    const IntSize& aSize, _D3DFORMAT aFormat, uint32_t aBPP) {
  RefPtr<IDirect3DSurface9> surface;
  D3DLOCKED_RECT lockedRect;
  RefPtr<IDirect3DTexture9> texture =
      InitTextures(aDeviceManager, aSize, aFormat, surface, lockedRect);
  if (!texture) {
    return nullptr;
  }
  ...
  for (int y = 0; y < aSize.height; y++) {
    memcpy((char*)lockedRect.pBits + lockedRect.Pitch * y, aData + aStride * y,
           width);
  }
...
already_AddRefed<IDirect3DTexture9> TextureSourceD3D9::TextureToTexture(
    DeviceManagerD3D9* aDeviceManager, IDirect3DTexture9* aTexture,
    const IntSize& aSize, _D3DFORMAT aFormat) {
  if (!aDeviceManager) {
    return nullptr;
  }
...
DataTextureSourceD3D9::DataTextureSourceD3D9(IDirect3DDevice9* aDevice,
...
DataTextureSourceD3D9::DataTextureSourceD3D9(gfx::SurfaceFormat aFormat,
                                             ...
                                             IDirect3DTexture9* aTexture)
    : DataTextureSourceD3D9(aProvider->GetD3D9Device(), aFormat, aSize,
                            aTexture) {}
...
IDirect3DTexture9* DataTextureSourceD3D9::GetD3D9Texture() {
  return mIterating ? mTileTextures[mCurrentTile] : mTexture;
}

bool DataTextureSourceD3D9::Update(gfx::DataSourceSurface* aSurface,
  ...
  if ((mSize.width <= maxSize && mSize.height <= maxSize) ||
    ...
    if (mTexture) {
      ...
      if (currentDesc.Width != mSize.width ||
          currentDesc.Height != mSize.height || currentDesc.Format != format) {
        mTexture = nullptr;
    ...
    if (!mTexture) {
      ...
      mTexture =
          deviceManager->CreateTexture(mSize, format, D3DPOOL_DEFAULT, this);
      mIsTiled = false;
      if (!mTexture) {
        Reset();
        return false;
    ...
    nsIntRegion regionToUpdate =
        aDestRegion ? *aDestRegion
    ...
    if (mFormat == SurfaceFormat::A8) {
      ...
      srcTexture =
          deviceManager->CreateTexture(mSize, format, D3DPOOL_SYSTEMMEM, this);
      if (!srcTexture) {
        aSurface->Unmap();
        return false;
      }
      ...
      HRESULT hr = mDevice->CreateOffscreenPlainSurface(
          mSize.width, mSize.height, format, D3DPOOL_SYSTEMMEM,
          getter_AddRefs(srcSurface), nullptr);
      if (FAILED(hr)) {
        aSurface->Unmap();
        return false;
      }
    ...
    for (auto iter = regionToUpdate.RectIter(); !iter.Done(); iter.Next()) {
      ...
      uint8_t* src = map.mData + map.mStride * iterRect.y +
                     BytesPerPixel(aSurface->GetFormat()) * iterRect.x;
      uint8_t* dest = reinterpret_cast<uint8_t*>(rect.pBits) +
                      rect.Pitch * iterRect.y +
                      BytesPerPixel(aSurface->GetFormat()) * iterRect.x;

      for (int y = 0; y < iterRect.height; y++) {
        memcpy(dest + rect.Pitch * y, src + map.mStride * y,
               iterRect.width * bpp);
      }
    }
    ...
    for (auto iter = regionToUpdate.RectIter(); !iter.Done(); iter.Next()) {
      ...
      mDevice->UpdateSurface(srcSurface, &updateRect, destSurface, &point);
    }
  } else {
    mIsTiled = true;
    ...
    for (uint32_t i = 0; i < tileCount; i++) {
      ...
      unsigned char* data = aSurface->GetData() +
                            tileRect.y * aSurface->Stride() + tileRect.x * bpp;
      mTileTextures[i] =
          DataToTexture(deviceManager, data, aSurface->Stride(),
                        IntSize(tileRect.width, tileRect.height), format, bpp);
      if (!mTileTextures[i]) {
...
void DataTextureSourceD3D9::Reset() {
  mSize.width = 0;
  mSize.height = 0;
  mIsTiled = false;
  mTexture = nullptr;
...
IntRect DataTextureSourceD3D9::GetTileRect(uint32_t aTileIndex) const {
  uint32_t maxSize = GetMaxTextureSize();
...
D3D9TextureData::~D3D9TextureData() { MOZ_COUNT_DTOR(D3D9TextureData); }

D3D9TextureData* D3D9TextureData::Create(gfx::IntSize aSize,
                                         gfx::SurfaceFormat aFormat,
                                         TextureAllocationFlags aAllocFlags) {
  _D3DFORMAT format = SurfaceFormatToD3D9Format(aFormat);
  RefPtr<DeviceManagerD3D9> deviceManager = DeviceManagerD3D9::Get();
  RefPtr<IDirect3DTexture9> d3d9Texture =
      deviceManager ? deviceManager->CreateTexture(aSize, format,
                                                   D3DPOOL_SYSTEMMEM, nullptr)
                    : nullptr;
  if (!d3d9Texture) {
...
TextureData* D3D9TextureData::CreateSimilar(
    LayersIPCChannel*, LayersBackend, TextureFlags aFlags,
    TextureAllocationFlags aAllocFlags) const {
  return D3D9TextureData::Create(mSize, mFormat, aAllocFlags);
}

void D3D9TextureData::FillInfo(TextureData::Info& aInfo) const {
  aInfo.size = mSize;
...
bool D3D9TextureData::Lock(OpenMode aMode) {
...
bool D3D9TextureData::Serialize(SurfaceDescriptor& aOutDescriptor) {
  mTexture.get()->AddRef();  // Release in TextureHostD3D9::TextureHostD3D9
  aOutDescriptor =
      SurfaceDescriptorD3D9(reinterpret_cast<uintptr_t>(mTexture.get()));
  return true;
}

already_AddRefed<gfx::DrawTarget> D3D9TextureData::BorrowDrawTarget() {
  ...
  if (mNeedsClearWhite) {
    dt->FillRect(Rect(0, 0, mSize.width, mSize.height),
...
bool D3D9TextureData::UpdateFromSurface(gfx::SourceSurface* aSurface) {
  MOZ_ASSERT(mD3D9Surface);
...
TextureHostD3D9::TextureHostD3D9(TextureFlags aFlags,
    ...
    : TextureHost(aFlags), mFormat(SurfaceFormat::UNKNOWN), mIsLocked(false) {
  mTexture = reinterpret_cast<IDirect3DTexture9*>(aDescriptor.texture());
  MOZ_ASSERT(mTexture);
  mTexture.get()
      ->Release();  // see AddRef in TextureClientD3D9::ToSurfaceDescriptor
  MOZ_ASSERT(mTexture);
...
bool DataTextureSourceD3D9::UpdateFromTexture(IDirect3DTexture9* aTexture,
...
void TextureHostD3D9::UpdatedInternal(const nsIntRegion* aRegion) {
  MOZ_ASSERT(mTexture);
  if (!mTexture) {
    return;
  }
  ...
  if (!mTextureSource) {
    mTextureSource = new DataTextureSourceD3D9(mProvider->GetD3D9Device(),
                                               mFormat, mSize, nullptr, mFlags);
    if (mFlags & TextureFlags::COMPONENT_ALPHA) {
...
IDirect3DDevice9* TextureHostD3D9::GetDevice() {
  if (mFlags & TextureFlags::INVALID_COMPOSITOR) {
    return nullptr;
  }
  if (mProvider) {
    return mProvider->GetD3D9Device();
  }
  return nullptr;
}
...
bool TextureHostD3D9::BindTextureSource(
...
void TextureHostD3D9::Unlock() {
  MOZ_ASSERT(mIsLocked);
  mIsLocked = false;
}

void TextureHostD3D9::DeallocateDeviceData() { mTextureSource = nullptr; }

DXGITextureHostD3D9::DXGITextureHostD3D9(
...
IDirect3DDevice9* DXGITextureHostD3D9::GetDevice() {
  if (mFlags & TextureFlags::INVALID_COMPOSITOR) {
    return nullptr;
  }
...
void DXGITextureHostD3D9::OpenSharedHandle() {
  MOZ_ASSERT(!mTextureSource);
  ...
  RefPtr<IDirect3DTexture9> texture;
  HRESULT hr = GetDevice()->CreateTexture(
      mSize.width, mSize.height, 1, D3DUSAGE_RENDERTARGET,
      SurfaceFormatToD3D9Format(mFormat), D3DPOOL_DEFAULT,
      getter_AddRefs(texture), (HANDLE*)&mHandle);
  if (FAILED(hr)) {
  ...
  mTextureSource =
      new DataTextureSourceD3D9(mFormat, mSize, mProvider, texture);

  return;
}

bool DXGITextureHostD3D9::BindTextureSource(
    CompositableTextureSourceRef& aTexture) {
  MOZ_ASSERT(mIsLocked);
  MOZ_ASSERT(mTextureSource);
  aTexture = mTextureSource;
...
void DXGITextureHostD3D9::Unlock() {
  MOZ_ASSERT(mIsLocked);
  mIsLocked = false;
}
...
void DXGITextureHostD3D9::DeallocateDeviceData() { mTextureSource = nullptr; }

DXGIYCbCrTextureHostD3D9::DXGIYCbCrTextureHostD3D9(
...
IDirect3DDevice9* DXGIYCbCrTextureHostD3D9::GetDevice() {
  if (mFlags & TextureFlags::INVALID_COMPOSITOR) {
    return nullptr;
  }
...
bool DXGIYCbCrTextureHostD3D9::Lock() {
  if (!mProvider) {
  ...
  if (!mTextureSources[0]) {
    ...
    if (FAILED(GetDevice()->CreateTexture(
            mSizeY.width, mSizeY.height, 1, 0, D3DFMT_A8, D3DPOOL_DEFAULT,
            getter_AddRefs(mTextures[0]), &mHandles[0]))) {
      return false;
    }

    if (FAILED(GetDevice()->CreateTexture(
            mSizeCbCr.width, mSizeCbCr.height, 1, 0, D3DFMT_A8, D3DPOOL_DEFAULT,
    ...
    mTextureSources[0] = new DataTextureSourceD3D9(SurfaceFormat::A8, mSize,
                                                   mProvider, mTextures[0]);
    mTextureSources[1] = new DataTextureSourceD3D9(SurfaceFormat::A8, mSize,
                                                   mProvider, mTextures[1]);
    mTextureSources[2] = new DataTextureSourceD3D9(SurfaceFormat::A8, mSize,
                                                   mProvider, mTextures[2]);
...
void DXGIYCbCrTextureHostD3D9::Unlock() {
  MOZ_ASSERT(mIsLocked);
  mIsLocked = false;
}

bool DXGIYCbCrTextureHostD3D9::BindTextureSource(
    CompositableTextureSourceRef& aTexture) {
  MOZ_ASSERT(mIsLocked);
