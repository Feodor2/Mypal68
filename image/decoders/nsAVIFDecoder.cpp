...
nsAVIFDecoder::DecodeResult nsAVIFDecoder::Decode(
  ...
  DecodeResult r = AOMDecoder::Create(std::move(parser), decoder);
  ...
  MOZ_LOG(sAVIFLog, LogLevel::Debug,
          ("[this=%p] Decoder%s->Decode() %s", this, "AOM",
...
//__asm("int3");
  ...
  if (0 && mCMSMode != eCMSMode_Off) {
    ...
    if (parsedImg.icc_colour_information.data) {
      ...
      MOZ_LOG(sAVIFLog, LogLevel::Error,
        ("[this=%p] TODO: qcms_profile_create_cicp"
         "failing",
         this));
      return AsVariant(NonDecoderResult::InvalidCICP);
...
bool nsAVIFDecoder::IsDecodeSuccess(const DecodeResult& aResult) {
  if (aResult.is<AOMResult>()) {
    return aResult == DecodeResult(AOMResult(AOM_CODEC_OK));
  }
  return false;
}
