...
public:
  ...
  IProtocol(ProtocolId aProtoId, Side aSide)
      : mId(0), mProtocolId(aProtoId), mSide(aSide),
        mLinkStatus(LinkStatus::Inactive), mLifecycleProxy(nullptr),
        mManager(nullptr), mToplevel(nullptr) {}
...
protected:
  ...
  // actor pointers.
  virtual void
  AllManagedActors(nsTArray<RefPtr<ActorLifecycleProxy>> &aActors) const = 0;
...
private:
  ...
  template <class T> using IDMap = nsDataHashtable<nsUint32HashKey, T>;
...
public:
  ...
  Endpoint()
      : mValid(false), mMode(static_cast<mozilla::ipc::Transport::Mode>(0)),
        mMyPid(0), mOtherPid(0) {}

  Endpoint(const PrivateIPDLInterface &, mozilla::ipc::Transport::Mode aMode,
           TransportDescriptor aTransport, ProcessId aMyPid,
           ProcessId aOtherPid)
      : mValid(true), mMode(aMode), mTransport(aTransport), mMyPid(aMyPid),
        mOtherPid(aOtherPid) {}

  Endpoint(Endpoint &&aOther)
      : mValid(aOther.mValid), mTransport(aOther.mTransport),
        mMyPid(aOther.mMyPid), mOtherPid(aOther.mOtherPid) {
    if (aOther.mValid) {
      mMode = aOther.mMode;
    }
    aOther.mValid = false;
  ...
  bool Bind(PFooSide *aActor) {
    ...
    if (!aActor->Open(std::move(transport), mOtherPid, XRE_GetIOMessageLoop(),
                      mMode == Transport::MODE_SERVER ? ParentSide
                                                      : ChildSide)) {
      return false;
    }
    mValid = false;
 ...
 * a browser crash.
 */
template <class PFooSide> class ManagedEndpoint {
public:
...
template <> struct ParamTraits<mozilla::ipc::ActorHandle> {
...
template <class PFooSide> struct ParamTraits<mozilla::ipc::Endpoint<PFooSide>> {
