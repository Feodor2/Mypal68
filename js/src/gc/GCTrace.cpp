...
static inline void TraceEvent(FILE* file, GCTraceEvent event,
                              uint64_t payload = 0, uint8_t extra = 0) {
...
static inline void TraceAddress(FILE* file, const void* p) {
  TraceEvent(file, TraceDataAddress, uint64_t(p));
}

static inline void TraceInt(FILE* file, uint32_t data) {
...
bool GCTrace::initTrace(GCRuntime& gc) {
...
bool GCTrace::traceEnabled() { return gcTraceFile != nullptr; }

void GCTrace::traceNurseryAlloc(Cell* thing, size_t size) {
  if (thing) {
...
void GCTrace::traceNurseryAlloc(Cell* thing, AllocKind kind) {
  if (thing) {
    TraceEvent(gcTraceFile, TraceEventNurseryAlloc, uint64_t(thing),
               uint8_t(kind));
  }
...
void js::gc::GCTrace::maybeTraceGroup(ObjectGroup* group) {
...
void GCTrace::traceTypeNewScript(ObjectGroup* group) {
  const size_t bufLength = 128;
  static char buffer[bufLength];

  JSAtom* funName =
      group->newScriptDontCheckGeneration()->function()->displayAtom();
  if (!funName) {
    return;
  }
...
void GCTrace::traceCreateObject(JSObject* object) {
  if (!gcTraceFile) {
    return;
  }
