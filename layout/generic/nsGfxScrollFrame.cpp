...
#undef NEEDS_REFLOW
  ...
  nsXULScrollFrame* NS_NewXULScrollFrame(PresShell * aPresShell,
    ...
    return new (aPresShell) nsXULScrollFrame(
        aStyle, aPresShell->GetPresContext(), aIsRoot, aClipAllDescendants);
  }
  ...
  nsMargin ScrollFrameHelper::GetDesiredScrollbarSizes(nsBoxLayoutState *
                                                       aState) {
  ...
  void ScrollFrameHelper::ScrollByLine(
    ...
    if (isHorizontal) {
      ...
      if (GetLineScrollAmount().height * delta.y >
  ...
  void ScrollFrameHelper::ThumbMoved(nsScrollbarFrame * aScrollbar,
    ...
    if (isHorizontal) {
      dest.x =
          IsPhysicalLTR() ? aNewPos : aNewPos - GetLayoutScrollRange().width;
    } else {
      dest.y = aNewPos;
    }
  ...
  nsresult nsXULScrollFrame::CreateAnonymousContent(nsTArray<ContentInfo> &
                                                    aElements) {
    return mHelper.CreateAnonymousContent(aElements);
  }
  ...
  void nsXULScrollFrame::RemoveFrame(ChildListID aListID,
                                     nsIFrame * aOldFrame) {
  ...
  ComputeBezierAnimationSettingsForOrigin(nsAtom * aOrigin) {
    ...
    // Duration should be at least as long as the intervals -> ratio is at least
    // 1
  ...
  void ScrollFrameHelper::AsyncScroll::InitSmoothScroll(
    ...
    if (!aOrigin || aOrigin == nsGkAtoms::restore ||
      ...
      // something changes we should never have aOrigin == nsGkAtoms::restore
      // here.
  ...
  void ScrollFrameHelper::AsyncSmoothMSDScrollCallback(
    ...
    NS_ASSERTION(
        aInstance->mAsyncSmoothMSDScroll,
   ...
   */
  static nscoord ClampAndAlignWithPixels(
      nscoord aDesired, nscoord aBoundLower, nscoord aBoundUpper,
      nscoord aDestLower, nscoord aDestUpper, nscoord aAppUnitsPerPixel,
    ...
    nscoord aligned = aRes == 0.0
                          ? 0.0
                          : NSToCoordRoundWithClamp(nearestLayerVal *
  ...
  static nsPoint ClampAndAlignWithLayerPixels(
      ...
      const nsPoint& aCurrent, nscoord aAppUnitsPerPixel,
  ...
  void ScrollFrameHelper::ScheduleSyntheticMouseMove() {
    if (!mScrollActivityTimer) {
      mScrollActivityTimer =
          NS_NewTimer(mOuter->PresContext()->Document()->EventTargetFor(
              TaskCategory::Other));
      if (!mScrollActivityTimer) {
        return;
      }
  ...
  void ScrollFrameHelper::ScrollToImpl(nsPoint aPt, const nsRect& aRange,
    ...
    if (nsLayoutUtils::AsyncPanZoomEnabled(mOuter) &&
      ...
      // skip the paint.
      // Because of bug 1264297, we also don't do paint-skipping for elements
      // with perspective, because the displayport may not have captured
      // everything that needs to be painted. So even if the final tile-aligned
      // displayport is the same, we force a repaint for these elements. Bug
      // 1254260 tracks fixing this properly.
      nsRect displayPort;
        ...
        if (!apzDisabled && !HasPluginFrames()) {
          if (LastScrollOrigin() == nsGkAtoms::apz) {
            ...
            if (manager) {
              ...
              MOZ_ASSERT(
                  success);  // we have a displayport, we better have an ID
  ...
  void ScrollFrameHelper::AppendScrollPartsTo(
      nsDisplayListBuilder * aBuilder, const nsDisplayListSet& aLists,
      bool aCreateLayer, bool aPositioned) {
    const bool overlayScrollbars =
        LookAndFeel::GetInt(LookAndFeel::IntID::UseOverlayScrollbars) != 0;

    AutoTArray<nsIFrame*, 3> scrollParts;
    for (nsIFrame* kid : mOuter->PrincipalChildList()) {
      if (kid == mScrolledFrame || (kid->IsAbsPosContainingBlock() ||
                                    overlayScrollbars) != aPositioned) {
        continue;
      }
    ...
    for (uint32_t i = 0; i < scrollParts.Length(); ++i) {
      ...
      // giant layer covering the whole scrollport if both scrollbars are
      // visible.
  ...
  nsRect ScrollFrameHelper::ExpandRectToNearlyVisible(const nsRect& aRect)
      const {
    ...
    nscoord vertShift =
        StaticPrefs::layout_framevisibility_numscrollportheights() *
        aRect.height;
  ...
  static void ClipItemsExceptCaret(
      nsDisplayList * aList, nsDisplayListBuilder * aBuilder,
      nsIFrame * aClipFrame, const DisplayItemClipChain* aExtraClip,
    ...
    for (nsDisplayItem* i = aList->GetBottom(); i; i = i->GetAbove()) {
      ...
      if (children) {
        ClipItemsExceptCaret(children, aBuilder, aClipFrame, aExtraClip,
                             aCache);
      }
    }
  }

  static void ClipListsExceptCaret(
      nsDisplayListCollection * aLists, nsDisplayListBuilder * aBuilder,
      nsIFrame * aClipFrame, const DisplayItemClipChain* aExtraClip) {
    nsDataHashtable<nsPtrHashKey<const DisplayItemClipChain>,
  ...
  void ScrollFrameHelper::BuildDisplayList(nsDisplayListBuilder * aBuilder,
    ...
    if (ignoringThisScrollFrame) {
      ...
      bool addScrollBars = mIsRoot && mWillBuildScrollableLayer &&
  ...
  void ScrollFrameHelper::MaybeAddTopLayerItems(
      nsDisplayListBuilder * aBuilder, const nsDisplayListSet& aLists) {
    if (mIsRoot) {
  ...
  bool ScrollFrameHelper::DecideScrollableLayer(
      nsDisplayListBuilder * aBuilder, nsRect * aVisibleRect,
      nsRect * aDirtyRect, bool aSetBase, bool* aDirtyRectHasBeenOverriden) {
    ...
    if (aBuilder->IsPaintingToWindow()) {
      if (aSetBase) {
          ...
          if (usingDisplayPort) {
            ...
            if (rootPresContext) {
              ...
              if (rootFrame) {
                ...
                // incorporate the APZ callback transforms into this. Most of
                // the time those transforms are negligible, but in some cases
                // (e.g. when a zoom is applied on an overflow:hidden document)
                // it is not (see bug 1280013).
                ...
                // 8). Instead, we translate rootCompBounds so as to maximize
                // the overlap with displayportBase, and *then* do the
                // intersection.
                if (rootCompBounds.x > displayportBase.x &&
                  ...
                  // same. This will line up the right edge of the two rects,
                  // and might mean that rootCompbounds.x is smaller than
                  // displayportBase.x. We can avoid that by taking the min of
                  // the x delta and XMost() delta, but it doesn't really matter
                  // because the intersection between the two rects below will
                  // end up the same.
                  rootCompBounds.x -=
                      (rootCompBounds.XMost() - displayportBase.XMost());
        ...
        nsLayoutUtils::SetDisplayPortBase(mOuter->GetContent(),
                                          displayportBase);
      }
  ...
  bool ScrollFrameHelper::IsRectNearlyVisible(const nsRect& aRect) const {
    ...
    return aRect.Intersects(ExpandRectToNearlyVisible(
        usingDisplayport ? displayPort : mScrollPort));
  }
  ...
  OverscrollBehaviorInfo ScrollFrameHelper::GetOverscrollBehaviorInfo() const {
    ...
    return OverscrollBehaviorInfo::FromStyleConstants(
        disp.mOverscrollBehaviorX, disp.mOverscrollBehaviorY);
  }
  ...
  nsRect ScrollFrameHelper::GetScrollRange(nscoord aWidth, nscoord aHeight)
      const {
   ...
   */
  static void CalcRangeForScrollBy(
      int32_t aDelta, nscoord aPos, float aNegTolerance, float aPosTolerance,
      nscoord aMultiplier, nscoord* aLower, nscoord* aUpper) {
    if (!aDelta) {
      *aLower = *aUpper = aPos;
      return;
    }
    *aLower = aPos - NSToCoordRound(aMultiplier * (aDelta > 0 ? aNegTolerance
                                                              : aPosTolerance));
    *aUpper = aPos + NSToCoordRound(aMultiplier * (aDelta > 0 ? aPosTolerance
                                                              : aNegTolerance));
  }
  ...
  void ScrollFrameHelper::ScrollBy(nsIntPoint aDelta, ScrollUnit aUnit,
    ...
    CalcRangeForScrollBy(aDelta.x, newPos.x, negativeTolerance,
                         positiveTolerance, deltaMultiplier.width, &rangeLowerX,
                         &rangeUpperX);
    CalcRangeForScrollBy(aDelta.y, newPos.y, negativeTolerance,
                         positiveTolerance, deltaMultiplier.height,
  ...
  static void AddToListIfHeaderFooter(
      nsIFrame * aFrame, nsIFrame * aScrollPortFrame, const nsRect& aScrollPort,
      nsTArray<TopAndBottom>& aList) {
    nsRect r = aFrame->GetRectRelativeToSelf();
    r = nsLayoutUtils::TransformFrameRectToAncestor(aFrame, r,
                                                    aScrollPortFrame);
  ...
  static nsSize GetScrollPortSizeExcludingHeadersAndFooters(
    ...
    footerTop =
        std::max(aScrollPort.height - aScrollPort.height / 3, footerTop);
    ...
    if (GetLogicalVisualViewportOffset() == mLastPos ||
      ...
      if (mRestorePos != mLastPos /* GetLogicalVisualViewportOffset() */ ||
        ...
        if (state == LoadingState::Loading || NS_SUBTREE_DIRTY(mOuter)) {
          ...
          // incrementally. So re-get the scroll position for the next
          // iteration, it might not be exactly equal to mRestorePos due to
  ...
  nsresult ScrollFrameHelper::CreateAnonymousContent(
    ...
    if (!mIsRoot) {
      if (mOuter->StyleUIReset()->mScrollbarWidth ==
...

    {
      ...
      if (canHaveHorizontal) {
        ...
        mHScrollbarContent =
            MakeScrollbar(nodeInfo, /* aVertical */ false, key);
    ...
    if (isResizable) {
      ...
      NS_TrustedNewXULElement(getter_AddRefs(mResizerContent),
                              nodeInfo.forget());
  ...
  void ScrollFrameHelper::CurPosAttributeChanged(nsIContent * aContent,
    ...
    NS_ASSERTION(
        (mHScrollbarBox && mHScrollbarBox->GetContent() == aContent) ||
  ...
  bool nsXULScrollFrame::AddRemoveScrollbar(nsBoxLayoutState & aState,
    ...
    if (aHorizontal) {
      ...
      bool fit = AddRemoveScrollbar(
          hasHorizontalScrollbar, mHelper.mScrollPort.y,
          mHelper.mScrollPort.height, hSize.height, aOnRightOrBottom, aAdd);
  ...
  bool ScrollFrameHelper::IsScrollingActive(nsDisplayListBuilder * aBuilder)
      const {
    const nsStyleDisplay* disp = mOuter->StyleDisplay();
    if (disp->mWillChange.bits & StyleWillChangeBits::SCROLL &&
  ...
  /**
   * Reflow the scroll area if it needs it and return its size. Also determine
   * if the reflow will cause any of the scrollbars to need to be reflowed.
   */
  ...
  void ScrollFrameHelper::FinishReflowForScrollbar(
   ...
   public:
    ...
    ~AutoMinimumScaleSizeChangeDetector() {
      if (mPreviousMinimumScaleSize != mHelper->mMinimumScaleSize ||
          mPreviousIsUsingMinimumScaleSize !=
              mHelper->mIsUsingMinimumScaleSize) {
  ...
  bool ScrollFrameHelper::ReflowFinished() {
    ...
    if (mMayHaveDirtyFixedChildren) {
      ...
      for (nsIFrame* fixedChild =
        ...
        mOuter->PresShell()->FrameNeedsReflow(
            fixedChild, IntrinsicDirty::Resize, NS_FRAME_HAS_DIRTY_CHILDREN);
      }
    }
    ...
    nscoord maxY =
        scrolledContentRect.YMost() - scrollClampingScrollPort.height;
    ...
    // FIXME(emilio): Why this instead of mHScrollbarContent /
    // mVScrollbarContent?
  ...
  bool ScrollFrameHelper::ComputeCustomOverflow(nsOverflowAreas &
                                                aOverflowAreas) {
    nsIScrollableFrame* sf = do_QueryFrame(mOuter);
  ...
  void ScrollFrameHelper::AdjustScrollbarRectForResizer(
    ...
    if ((aDirection == ScrollDirection::eVertical ? aRect.width
                                                  : aRect.height) == 0) {
      return;
    }
  ...
  void ScrollFrameHelper::LayoutScrollbars(nsBoxLayoutState & aState,
      ...
      if (hasResizer) {
        ...
        nscoord hScrollbarHeight =
            mHScrollbarBox ? mHScrollbarBox->GetXULPrefSize(aState).height
                           : defaultSize;
        r.height = std::max(std::max(r.height, hScrollbarHeight),
                            resizerMinSize.height);
        if (aContentArea.y == mScrollPort.y) {
          r.y = aContentArea.YMost() - r.height;
        }
    ...
    if (!LookAndFeel::GetInt(
            LookAndFeel::IntID::AllowOverlayScrollbarsOverlap)) {
  ...
  void ScrollFrameHelper::SetScrollbarEnabled(Element * aElement,
    ...
    MOZ_ASSERT(ShellIsAlive(weakShell),
               "pres shell was destroyed by scrolling");
  }
  ...
  void ScrollFrameHelper::SetCoordAttribute(Element * aElement, nsAtom * aAtom,
    ...
    if (aElement->AttrValueIs(kNameSpaceID_None, aAtom, newValue,
                              eCaseMatters)) {
      return;
    }
  ...
  static void ReduceRadii(nscoord aXBorder, nscoord aYBorder,
   ...
   * have scrollbars.
   */
  bool ScrollFrameHelper::GetBorderRadii(
      const nsSize& aFrameSize, const nsSize& aBorderArea, Sides aSkipSides,
  ...
  nsRect ScrollFrameHelper::GetUnsnappedScrolledRectInternal(
      const nsRect& aScrolledFrameOverflowArea, const nsSize& aScrollPortSize)
      const {
    return nsLayoutUtils::GetScrolledRect(
        mScrolledFrame, aScrolledFrameOverflowArea, aScrollPortSize,
        GetScrolledFrameDir());
  }
  ...
  nscoord ScrollFrameHelper::GetCoordAttribute(
      nsIFrame * aBox, nsAtom * aAtom, nscoord aDefaultValue,
  ...
  UniquePtr<PresState> ScrollFrameHelper::SaveState() const {
    ...
    bool isInSmoothScroll =
        IsProcessingAsyncScroll() || mLastSmoothScrollOrigin;
  ...
  void ScrollFrameHelper::RestoreState(PresState * aState) {
    ...
    mLastPos =
        mScrolledFrame ? GetLogicalVisualViewportOffset() : nsPoint(0, 0);
  ...
  nsRect ScrollFrameHelper::GetScrollRangeForUserInputEvents() const {
    ...
    // The results in the same notion of scroll range that APZ uses (the
    // combined effect of FrameMetrics::CalculateScrollRange() and
  ...
  static void AppendScrollPositionsForSnap(
      ...
      const Maybe<nsRect>& aSnapport, WritingMode aWritingModeOnScroller,
  ...
  static nsMargin ResolveScrollPaddingStyle(
    ...
    return nsMargin(ResolveScrollPaddingStyleValue(aScrollPaddingStyle,
                                                   eSideTop, aScrollPortSize),
                    ResolveScrollPaddingStyleValue(aScrollPaddingStyle,
                                                   eSideRight, aScrollPortSize),
                    ResolveScrollPaddingStyleValue(
                        aScrollPaddingStyle, eSideBottom, aScrollPortSize),
                    ResolveScrollPaddingStyleValue(aScrollPaddingStyle,
                                                   eSideLeft, aScrollPortSize));
  }
  ...
  bool ScrollFrameHelper::GetSnapPointForDestination(
      ScrollUnit aUnit, const nsPoint& aStartPos, nsPoint& aDestination) {
