...
function getButtonByAttribute(aName, aValue) {
  if (video.openOrClosedShadowRoot) {
    return video.openOrClosedShadowRoot.firstChild.querySelector(
      `[${aName}="${aValue}"]`
    );
  }
  ...
  return contentDocument.getAnonymousElementByAttribute(
    videocontrols,
    aName,
    aValue
  );
...
function testPause(aEvent) {
  ...
  let playButton = getButtonByAttribute("class", "playButton");
  ok(
    playButton.getAttribute("paused") == "true",
    "Play button is paused again"
  );
  ...
  ok(
    bar1.r >= MAX_COLOR && bar1.g >= MAX_COLOR && bar1.b >= MAX_COLOR,
    "Bar 1 is white"
  );
  ...
  ok(
    bar2.r >= MAX_COLOR && bar2.g >= MAX_COLOR && bar2.b <= MIN_COLOR,
    "Bar 2 is yellow"
  );
  ...
  ok(
    bar3.r <= MIN_COLOR && bar3.g >= MAX_COLOR && bar3.b >= MAX_COLOR,
    "Bar 3 is Cyan"
  );
  ...
  ok(
    bar4.r <= MIN_COLOR && bar4.g >= MAX_COLOR && bar4.b <= MIN_COLOR,
    "Bar 4 is Green"
  );
  ...
  ok(
    bar5.r >= MAX_COLOR && bar5.g <= MIN_COLOR && bar5.b >= MAX_COLOR,
    "Bar 5 is Purple"
  );
  ...
  ok(
    bar6.r >= MAX_COLOR && bar6.g <= MIN_COLOR && bar6.b <= MIN_COLOR,
    "Bar 6 is Red"
  );
  ...
  ok(
    bar7.r <= MIN_COLOR && bar7.g <= MIN_COLOR && bar7.b >= MAX_COLOR,
    "Bar 7 is Blue"
  );
