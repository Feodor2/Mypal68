...
Maybe<nsCString> SVCB::GetAlpn(bool aNoHttp2) const {
  ...
  for (const auto& value : mSvcFieldValue) {
    if (value.mValue.is<SvcParamAlpn>()) {
      ...
      if (!alpnValue.IsEmpty()) {
        alpn->Assign(SelectAlpnFromAlpnList(alpnValue, aNoHttp2));
      }
      return alpn;
    }
  }
...
void SVCB::GetIPHints(nsTArray<mozilla::net::NetAddr>& aAddresses) const {
...
DNSHTTPSSVCRecordBase::GetServiceModeRecordInternal(
    bool aNoHttp2, const nsTArray<SVCB>& aRecords) {
  ...
  for (const SVCB& record : aRecords) {
    ...
    Maybe<nsCString> alpn = record.GetAlpn(aNoHttp2);
