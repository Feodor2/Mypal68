...
function onSave() {
  ...
  if (params.typeCol == prefs.PREF_BOOL) {
    // строки для значения здесь нет, поэтому при отсутствии имени возврат в диалог
    if (!name) return;

    if (valueElem.selectedIndex < 0) {
      // значение из списка не выбрано, поэтому попытаться использовать его из хвоста и вернуться в диалог
...
//        default:  ничего не менять
    ...
    // если заполнено только одно из полей - принять меры...
    if (!name) {
      name = value;
      value = "";
    }
...
//  returns false, fires an invalid event at the element, and (if the event isn't
