...
public:
  ...
  template <int N> void AssertOwnership(const char (&aMsg)[N]) const {
...
#define NSCAP_LOG_ASSIGNMENT(_c, _p)                                           \
  if (_p != nullptr)                                                           \
  NS_LogCOMPtrAddRef((_c), ToSupports(_p))

#define NSCAP_LOG_RELEASE(_c, _p)                                              \
  if (_p)                                                                      \
  NS_LogCOMPtrRelease((_c), ToSupports(_p))
...
#define NS_INLINE_DECL_REFCOUNTING_ONEVENTTARGET(_class, ...)                  \
  NS_INLINE_DECL_REFCOUNTING_WITH_DESTROY(                                     \
      _class, delete (this), NS_DECL_OWNINGEVENTTARGET, __VA_ARGS__)
...
#define NS_INTERFACE_TABLE_END NS_INTERFACE_TABLE_END_WITH_PTR(this)
...
#define NS_IMPL_QUERY_BODY_AMBIGUOUS(_interface, _implClass)                   \
  if (aIID.Equals(NS_GET_IID(_interface)))                                     \
    foundInterface =                                                           \
        static_cast<_interface *>(static_cast<_implClass *>(this));            \
  else
