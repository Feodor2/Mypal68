...
void nsComponentManagerImpl::RegisterCIDEntryLocked(
  ...
  mFactories.WithEntryHandle(aEntry->cid, [&](auto&& entry) {
    if (entry) {
      ...
      Monitor2AutoUnlock unlock(mLock);
      LogMessage(
          "While registering XPCOM module %s, trying to re-register CID '%s' "
...
void nsComponentManagerImpl::ManifestComponent(ManifestProcessingContext& aCx,
  ...
  Maybe<Monitor2AutoLock> lock(std::in_place, mLock);
...
Maybe<EntryWrapper> nsComponentManagerImpl::LookupByCID(const nsID& aCID) {
  return LookupByCID(Monitor2AutoLock(mLock), aCID);
}

Maybe<EntryWrapper> nsComponentManagerImpl::LookupByCID(const Monitor2AutoLock&,
                                                        const nsID& aCID) {
...
Maybe<EntryWrapper> nsComponentManagerImpl::LookupByContractID(
  ...
  return LookupByContractID(Monitor2AutoLock(mLock), aContractID);
}

Maybe<EntryWrapper> nsComponentManagerImpl::LookupByContractID(
    const Monitor2AutoLock&, const nsACString& aContractID) {
...
// This should only ever be called within the monitor!
void nsComponentManagerImpl::RemovePendingService(Monitor2AutoLock& aLock,
...
nsresult nsComponentManagerImpl::GetServiceLocked(Maybe<Monitor2AutoLock>& aLock,
                                                  EntryWrapper& aEntry,
                                                  const nsIID& aIID,
                                                  void** aResult) {
...
already_AddRefed<nsIFactory> nsFactoryEntry::GetFactory() {
  ...
  if (!mFactory) {
    ...
    Monitor2AutoLock lock(nsComponentManagerImpl::gComponentManager->mLock);
